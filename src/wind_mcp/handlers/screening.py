...
def handle_screen(params: ScreenInput) -> list[dict]:
    ...
    result = run_wind_sync(session.w.weqs, params.filter_name)
...
def handle_dynamic_screen(params: DynamicScreenInput) -> list[dict]:
    ...
    codes = resolve_universe(params.universe)
    if not codes:
        return []
    ...
    result = run_wind_sync(session.w.wss, codes_str, fields_str)
    data = parse_wss(result)

    # Step 4: Apply filters
    if params.filters:
        data = apply_filters(data, params.filters)
